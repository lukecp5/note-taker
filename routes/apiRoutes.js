...
router.delete("/notes/:id", (req, res) => {
  // Path to the JSON database for writing to the file. It gets imported at the top as const db
  let jsonPath = path.join(__dirname, "../db/db.json");
  // For each item in the notes db, check to see if the ID of the delete request matches the id of a note, if It does, delete the note
  ...
  // Write the newly updated notes to the db.json file, after removing the selected note from the database
  ...
  // Log the deleted note's body to the console, and return the database as JSON to the client
